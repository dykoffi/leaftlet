...
import { Grid, Stack, Text, TextInput, Timeline } from '@mantine/core'
    ...
    <Grid gutter={0}>
      <Grid.Col span={3}>
        ...
        <Stack p={25} spacing={25}>
          ...
          <Timeline style={{ maxHeight: "100vh" }} bulletSize={30} lineWidth={10}>
            {
              gtfs.stops.map((stop: any, index: number) =>
                <Timeline.Item key={index} bullet={<IconCurrentLocation />} title={stop.name}>
      ...
      <Grid.Col span={9}>
        <MapContainer boxZoom style={{ height: "100vh", padding: 0 }} center={[5.37, -4]} zoom={13}>
          ...
          <Polyline pathOptions={{ color: "lime", opacity: 0.6, weight: 15 }} positions={gtfs.shapes} />
        </MapContainer>
      </Grid.Col>
    </Grid>
  )
